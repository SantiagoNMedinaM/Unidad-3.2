...
class Lista:
    ...
    def tesorero(self, dni):
        aux = self.__comienzo
        while aux != None:
            if dni in aux.getDato().getCUIL():
                print("El gasto en sueldo de la persona ingresada es de:{}$".format(aux.getDato().getcalculoSueldo()))
            aux = aux.getSiguiente()
    def modificarBasico(self,dni,basico):
        aux = self.__comienzo
        while aux != None:
            if dni in aux.getDato().getCUIL():
                aux.getDato().modificarBasico(basico)
            aux = aux.getSiguiente()
    def modificarPorcentajeporcargo(self,dni,nuevo):
        aux = self.__comienzo
        while aux != None:
            if dni in aux.getDato().getCUIL():
                aux.getDato().modificarPorcentajeporcargo(nuevo)
            aux = aux.getSiguiente()
    def modificarPorcentajeporcategoría(self,dni, nuevoPorcentaje):
        aux = self.__comienzo
        while aux != None:
            if dni in aux.getDato().getCUIL():
                aux.getDato().modificarPorcentajeporcategoría(nuevoPorcentaje)
            aux = aux.getSiguiente()
    def modificarImporteExtra(self,dni, nuevoImporteExtra):
        aux = self.__comienzo
        while aux != None:
            if dni in aux.getDato().getCUIL():
                aux.getDato().modificarImporteExtra(nuevoImporteExtra)